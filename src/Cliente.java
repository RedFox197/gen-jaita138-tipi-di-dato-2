...
public class Cliente {
    public static void main(String[] args) {
        ...
        LocalDate dataNascita = LocalDate.parse(scanner.nextLine(), dateFormatter);
        ...
        LocalDateTime dataIscrizione = LocalDateTime.parse(scanner.nextLine(), dateTimeFormatter);

        System.out.println("RIEPILOGO DATI INSERITI: ");
        System.out.printf(
                "Nome: %s\nCognome: %s\nData di Nascita: %s\nNumero di Telefono: %s\nNote: %s\nEmail: %s\nPassword: %s\nUsername: %s\nData di Iscrizione: %s\n",
                nome, cognome, dataNascita.format(dateFormatter), nTelefono, note, email, password, username, dataIscrizione.format(dateTimeFormatter)
        );
        scanner.close();
    }
}
